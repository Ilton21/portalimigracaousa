  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-white text-gray-800">
      <header className="flex justify-between items-center px-6 py-4 bg-white shadow-sm sticky top-0 z-50">
        ...
        <button className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-full">Entrar</button>
      </header>

      <section className="text-center px-6 py-20 bg-blue-100">
        <motion.h2 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="text-4xl font-extrabold text-blue-700 mb-4">
        ...
        <button className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-3 rounded-full text-lg">Comece Aqui</button>
      </section>
      ...
      <section id="recursos" className="px-6 py-16 bg-blue-50">
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {[
            { icon: <Globe2 className="w-10 h-10 text-blue-600 mb-3" />, title: "Documentação e Vistos", desc: "Guias e informações atualizadas sobre processos migratórios e documentação." },
            { icon: <Users className="w-10 h-10 text-blue-600 mb-3" />, title: "Comunidades Locais", desc: "Descubra redes de apoio e associações próximas a você." },
            { icon: <MessageSquare className="w-10 h-10 text-blue-600 mb-3" />, title: "Atendimento e Orientação", desc: "Conecte-se com especialistas e voluntários prontos para ajudar." }
          ].map((item, i) => (
            <div key={i} className="rounded-2xl bg-white p-6 text-center shadow-md hover:shadow-lg transition">
              {item.icon}
      ...
      <section className="text-center py-16 px-6">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="max-w-2xl mx-auto">
          <Heart className="w-12 h-12 text-blue-600 mx-auto mb-4" />
          <h3 className="text-3xl font-semibold text-blue-700 mb-4">Juntos por uma nova vida</h3>
          <p className="text-gray-600 mb-6">Cada jornada é única. Nosso compromisso é oferecer informações, empatia e acolhimento a todos.</p>
          <button className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-3 rounded-full text-lg">Fale Conosco</button>
        </motion.div>
      </section>
